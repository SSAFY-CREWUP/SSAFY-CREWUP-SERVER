...
class CourseMapperTest {
    ...
    @Test
    @DisplayName("코스 목록 검색 테스트 (동적 쿼리 검증)")
    void selectCourseListTest() {
        // given
        // 데이터 2개 넣기 (하나는 EASY, 하나는 HARD)
        Course c1 = createDummyCourse("한강 러닝", "EASY");
        Course c2 = createDummyCourse("남산 업힐", "HARD");

        courseMapper.insertCourse(c1);
        courseMapper.insertCourse(c2);

        // when & then

        // 1. 전체 조회 (필터 없음)
        List<CourseListResponse> all = courseMapper.selectCourseList(null, null);
        assertThat(all).hasSize(2);

        // 2. 키워드 검색 ("한강")
        List<CourseListResponse> searchKeyword = courseMapper.selectCourseList("한강", null);
        assertThat(searchKeyword).hasSize(1);
        assertThat(searchKeyword.get(0).getTitle()).isEqualTo("한강 러닝");

        // 3. 난이도 필터링 ("HARD")
        List<CourseListResponse> searchDifficulty = courseMapper.selectCourseList(null, "HARD");
        assertThat(searchDifficulty).hasSize(1);
        assertThat(searchDifficulty.get(0).getTitle()).isEqualTo("남산 업힐");
    }
